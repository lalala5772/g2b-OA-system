...
**디자인 톤(2차 보정 — imweb "Cera — Mix" 템플릿 참고):** 딥 네이비 대신 **따뜻한 톤의 다크 차콜** 배경 + 파치먼트색 액센트로 재보정했습니다. 브랜드 워드마크("B2G — Automation")만 셀프호스팅한 서리프(Instrument Serif)로 표시하고, 한글이 섞인 본문·헤드라인·네비게이션 라벨은 전부 기존 산세리프 스택을 유지합니다 — Instrument Serif 등 서구권 서리프 폰트는 한글 글리프가 없어 브라우저가 임의의 폴백 서체로 대체하는데, 환경에 따라 필기체처럼 깨져 보이는 걸 실제로 확인해서 한글 텍스트에는 적용하지 않기로 결정했습니다. Pill 형태 버튼, em dash(—) 구두점 모티프, 얇은 헤어라인 구분선은 유지. (원래 설계서 초안의 "세리프 헤드라인 + 노이즈 텍스처" → all4land.com 톤(딥 네이비) → 지금의 다크 차콜, 순서로 보정되어 왔습니다. 참고 서비스의 로고·사진·문구는 그대로 쓰지 않습니다.)
