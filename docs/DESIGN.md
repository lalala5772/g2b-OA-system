...
ai_engine/
 ├─ main.py
 ├─ routers/ (files.py, company_files.py, ideas.py, documents.py, evidence.py, bids.py)
 ├─ services/
 │    ├─ file_parser/
...
### 메인 페이지 (기능 허브)
- 상단 얇은 네비게이션: 로고 — `아이디어 / 문서 / 증빙 / 나라장터 / 자료실` — 사용자 프로필(Google 프로필 이미지) / 로그아웃
- 히어로 영역: 대형 헤드라인, 스크롤/hover 시 미세한 트랜지션
- 본문: 4개의 대형 카드(타일) 그리드 (2x2, PC 기준) — hover 시 미세한 확대(scale 1.02) + 하단에 "Access —" 텍스트 노출
- 각 카드에 실시간 요약 배지(예: "신규 공고 2건") 삽입
- "회사 자료실"은 4개 기능 카드와 별도로 상단 네비게이션 아이콘(문서함 아이콘)으로 상시 노출

### 회사 자료실 페이지
- 좌측: 카테고리 필터(전체/도메인소개/증빙서류/재무자료/기타) — 얇은 세로 탭
- 중앙: 드래그앤드롭 업로드 영역 + 업로드 시 카테고리 선택 드롭다운
...
```
src/
 ├─ pages/ (Login, Dashboard, CompanyFilesPage, IdeaPage, DocumentPage, EvidencePage, BidPage)
